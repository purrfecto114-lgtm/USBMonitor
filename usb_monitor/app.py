...
class ConfigStore:
    ...
    def load(self) -> AppConfig:
        ...
        return AppConfig(
            ...
            # v1.1.1 边界防护: 配置文件也可能被构造恶意大值,同样限制
            log_max_bytes=max(min(as_int(raw.get("log_max_bytes"), 262_144, 10_000), 16_777_216), 10_000),
            log_backups=max(min(as_int(raw.get("log_backups"), 3, 0), 9), 0),
...
class LoggingManager:
    ...
    def _rotate_crash_log(self, crash_path: Path) -> None:
        ...
        v1.1.1: 轮转失败时记录诊断日志(不阻断写入),避免静默吞错导致磁盘涨满。
        """
        ...
        if oldest.exists():
            try:
                ...
                rotate_errors.append(f"unlink_old:{exc}")
        for index in range(self.CRASH_LOG_BACKUPS - 1, 0, -1):
            ...
            if src.exists():
                try:
                    ...
                    rotate_errors.append(f"replace_{index}:{exc}")
        try:
            crash_path.replace(base / "crash.log.1")
        except OSError as exc:
            rotate_errors.append(f"replace_current:{exc}")
            # 轮转失败兜底: 直接截断当前 crash.log,防止持续超限涨满磁盘
            try:
                crash_path.write_bytes(b"")
            except OSError:
                pass
        if rotate_errors:
            # 不走正常日志系统(可能正是它崩了),直接 stderr
            print(f"[USBMonitor] crash_log_rotate_partial_fail: {rotate_errors}", file=sys.stderr)
...
def merge_cli_config(args: argparse.Namespace, stored: AppConfig) -> AppConfig:
    ...
    if args.log_max_bytes is not None:
        # v1.1.1 边界防护: 上限 16MB/文件,下限 10KB;防止恶意 --log-max-bytes=999GB
        # 导致磁盘耗尽(每个日志类别×backups×16MB=48MB封顶,3类=144MB)。
        config.log_max_bytes = max(min(args.log_max_bytes, 16_777_216), 10_000)
    if args.log_backups is not None:
        # v1.1.1 边界防护: 上限 9 个备份,下限 0;防止 --log-backups=999999
        config.log_backups = max(min(args.log_backups, 9), 0)
