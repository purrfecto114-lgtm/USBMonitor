...
class HookRunner:
    ...
    def _fire(self, rule: HookRule, volume: VolumeInfo) -> None:
        ...
        # SECURITY (v1.1.1): BatBadBut / CVE-2024-24576 防护。
        # shell=False 在 Windows 上若 executable 是 .bat/.cmd/.ps1，CreateProcessW
        # 会隐式拉起 cmd.exe 解析 argv，导致攻击者可通过构造 USB 卷标
        # (如 "FOO&calc.exe" 或 "BAR/../../evil.exe") 实现 RCE。
        # 参考: https://flatt.tech/research/posts/batbadbut-you-cant-securely-execute-commands-on-windows
        reject_reason = _validate_hook_command_safety(cmd)
        if reject_reason is not None:
            LOG.error("hook_command_rejected", extra={"rule": rule.name, "reason": reject_reason})
            return
...
# BatBadBut 防护: cmd.exe 元字符 + path traversal 序列。
# 这些字符在 .bat/.cmd/.ps1 executable 路径下即使 shell=False 仍可被 cmd.exe 解析。
# 同时拒绝 ../  防止 CmdHijack (https://hackingiscool.pl/cmdhijack-command-argument-confusion-with-path-traversal-in-cmd-exe)
_CMD_INJECTION_CHARS = frozenset("&|<>()%^")
_CMD_TRAVERSAL = "../"
_CMD_TRAVERSAL_WIN = "\\.."
# Windows CreateProcessW 隐式拉起 cmd.exe 的可执行后缀
_CMD_SHELL_SUFFIXES = (".bat", ".cmd", ".ps1", ".bat\\", ".cmd\\")


def _validate_hook_command_safety(cmd: Sequence[str]) -> str | None:
    """检查 hook command 是否存在 BatBadBut/CmdHijack 注入风险。

    返回 None 表示安全;返回字符串说明拒绝原因。
    """
    if not cmd:
        return "empty_command"
    executable = cmd[0]
    # 1. 拒绝 .bat/.cmd/.ps1 作为 executable —— CreateProcessW 会隐式拉起 cmd.exe
    exec_lower = executable.lower()
    for suffix in _CMD_SHELL_SUFFIXES:
        if exec_lower.endswith(suffix):
            return f"shell_script_executable:{suffix}"
    # 2. 对所有 token(含 {path}/{label} 替换后的)检查 cmd.exe 元字符与 path traversal
    for index, token in enumerate(cmd):
        for ch in token:
            if ch in _CMD_INJECTION_CHARS:
                return f"cmd_metachar:{ch!r} in argv[{index}]"
        if _CMD_TRAVERSAL in token or _CMD_TRAVERSAL_WIN in token:
            return f"path_traversal in argv[{index}]"
    return None